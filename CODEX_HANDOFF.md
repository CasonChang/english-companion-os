...
- **T1.1 — Core tables migration.** `sessions`, `learning_items`, `session_learning_items`, `mistake_events`, `review_events`, `weekly_reports`, `user_settings` exactly per §G (columns, PKs, FKs, uniques, indexes, CHECKs). *Accept:* applies cleanly to a fresh project; re-apply is idempotent-safe (migration tool guards).
- **T1.2 — Views + SRS function.** `v_weekly_activity`, `v_mistake_category_stats`, `v_due_reviews`; `apply_review_rating(item_id uuid, rating text)` implementing the §K ladder as `security definer` with an `auth.uid()` ownership check. *Accept:* unit-style SQL tests in `supabase/tests.sql` demonstrate each rating transition (again/hard/good/easy, mastery at level 5 + 2×good).
...
- **T2.1 — Scaffold + auth.** Vite app, Tailwind, HashRouter, supabase client from env, login page, session persistence, logout, route guard. *Accept:* wrong password errors gracefully; refresh stays logged in; logged-out users see only the login screen and no data requests fire.
- **T2.2 — Layout shell.** Responsive nav (bottom tabs ≤768 px / sidebar above), page skeletons, loading + empty states, dark mode via `prefers-color-scheme`. *Accept:* matches §I nav spec at 360 px and 1280 px.
- **T2.3 — Home.** Stat row (week minutes, sessions, weekly streak, due count), focus card, last-session card, 8-week mini chart, review CTA. *Accept:* numbers match seed data hand-computation.
