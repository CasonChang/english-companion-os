...
- **T2.7 — Progress.** ✅ Complete. The four §I charts via Recharts, mobile-scrollable. *Accept:* charts match seed data; no other charts added.
- **T2.8 — Pages deploy workflow.** ✅ Complete. `deploy-dashboard.yml`: build with repo-variable env, deploy on push to `main`; README badge + URL. *Accept:* live URL serves the app; hash routes survive refresh.
...
- **T4.2 — Conversation loop + scoring.** ✅ Complete. Sequential Q&A, skip/stop/timeout, LLM rating → `review_events` + `apply_review_rating` semantics (service-role variant), recap message, user rating override. *Accept:* Phase 4 criteria in §P.
- **T4.3 — Scheduling + settings chat.** ✅ Implementation complete; hosted verification pending. Cron driven by `user_settings` (time, timezone, enabled, count); natural-language settings updates ("move my review to 21:00"); zero-due silence; 7-day-idle gentle nudge (≤1/week). *Accept:* changing the DB row changes fire time with no code edits.
