...
**Phase 1 implementation is complete; Phase 2 Dashboard T2.1 is next.** Applying the migrations to the user's hosted Supabase project remains a user-facing checkpoint. The remaining external Phase 0 acceptance check is for the user to produce one real ChatGPT Project session export and validate it. Phases and acceptance criteria: `docs/MASTER_PLAN.md` §P. Work phases in order; within a phase, tasks below are sized for one PR each. Update this section's phase pointer when a phase completes.
